...
export class DryerService extends BaseService {
  ...
  constructor(platform: IKHomeBridgeHomebridgePlatform, accessory: PlatformAccessory, componentId: string, capabilities: string[],
    ...
    this.log.debug(`Adding DryerService to ${this.name}`);
    ...
    // InUse: dryer job is actively working (drying/cooling/etc)
    ...
    if (this.platform.config.ExposeContactSensorForDryers) {
      const contactSubtype = 'dryer-contact-sensor';
  ...
  async getActive(): Promise<CharacteristicValue> {
    return new Promise((resolve, reject) => {
      this.getStatus().then(success => {
        ...
        try {
          const machineState = this.deviceStatus.status.dryerOperatingState.machineState.value;
          this.log.debug(`Dryer machineState for ${this.name}: ${machineState}`);
          resolve(this.machineStateToActive(machineState));
        } catch (error) {
          this.log.error(`Missing dryerOperatingState.machineState from ${this.name}`);
          reject(new this.platform.api.hap.HapStatusError(this.platform.api.hap.HAPStatus.SERVICE_COMMUNICATION_FAILURE));
        }
      });
    });
  ...
  async getInUse(): Promise<CharacteristicValue> {
    return new Promise((resolve, reject) => {
      this.getStatus().then(success => {
        ...
        try {
          const jobState = this.deviceStatus.status.dryerOperatingState.dryerJobState.value;
          this.log.debug(`Dryer jobState for ${this.name}: ${jobState}`);
          resolve(this.jobStateToInUse(jobState));
        } catch (error) {
          this.log.error(`Missing dryerOperatingState.dryerJobState from ${this.name}`);
          reject(new this.platform.api.hap.HapStatusError(this.platform.api.hap.HAPStatus.SERVICE_COMMUNICATION_FAILURE));
        }
      });
    });
  ...
  public processEvent(event: ShortEvent): void {
    if (event.capability === 'dryerOperatingState') {
      if (event.attribute === 'machineState') {
        this.log.debug(`Event updating dryer machineState for ${this.name} to ${event.value}`);
