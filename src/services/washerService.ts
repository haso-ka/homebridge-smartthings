...
export class WasherService extends BaseService {
  ...
  async setDuration(value: CharacteristicValue) {
    this.log.debug(`WasherService setDuration(${value}) ignored for ${this.name} (read-only)`);
  }
  ...
  async pollValveState(): Promise<CharacteristicValue> {
    ...
    const operatingState = this.deviceStatus.status.washerOperatingState;
    if (!operatingState || !operatingState.machineState || !operatingState.washerJobState) {
      this.log.error(`Missing washerOperatingState from ${this.name}`);
      throw new this.platform.api.hap.HapStatusError(this.platform.api.hap.HAPStatus.SERVICE_COMMUNICATION_FAILURE);
    }

    const machineState = operatingState.machineState.value;
    const jobState = operatingState.washerJobState.value;
    this.log.debug(`Washer machineState=${machineState} jobState=${jobState} for ${this.name}`);
  ...
  private applyInUseTransition(inUse: number): void {
    ...
    if (nowInUse && (!wasInUse || !this.remainingSeeded)) {
      ...
      this.log.debug(`Washer cycle ended for ${this.name}: RemainingDuration 0`);
    }
  ...
  private seedRemainingDuration(): void {
    ...
    if (secs > 0) {
      ...
      this.log.debug(`Washer countdown seeded for ${this.name}: ${secs}s`);
    } else {
      this.log.debug(`Washer in use but no completionTime yet for ${this.name}; will retry`);
    }
  }
  ...
  async getRemainingDuration(): Promise<CharacteristicValue> {
    return new Promise((resolve) => {
      this.getStatus().then(success => {
        ...
        this.log.debug(`Washer RemainingDuration for ${this.name}: ${remainingSeconds}s`);
